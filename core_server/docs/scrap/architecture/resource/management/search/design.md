# Search (检索) 模块

该系统分阶段进行制造，该模块可与推荐系统配合.  
不支持使用UUID检索，应直接请求对应UUID的数据.  
接收以下内容：
- 关键词.
- 过滤.
- 排序.

---
## 阶段
- [核心功能](#核心功能)
- [拓展功能](#拓展功能)
- [边沿功能](#边沿功能)

**依赖**：
```mermaid
graph LR
    subgraph 核心功能
        filter[过滤]
        search_keyword[关键词搜索]
        condition_sort[条件排序]
    end
    subgraph 拓展功能
        search_keyword_restrict[搜索范围限定]
        recommendation_sort[推荐系统排序]
        get_same_kind_resource_data[get同类resource]
    end
    subgraph 边沿功能
        fuzzy_search[模糊搜索]
    end
    recommendation[推荐系统]
    recommendation_sort --> recommendation;
    fuzzy_search --- search_keyword;
    search_keyword --- search_keyword_restrict;
```
**决策树**：
```mermaid
graph LR
    had_sort_condition?{排序条件？}
    condition_sort[条件排序]
    recommendation_sort[推荐系统排序]
    had_sort_condition? --有--> condition_sort;
    had_sort_condition? --无--> recommendation_sort;
```
---

## 核心功能
### 关键词搜索
返回含有`关键词`的resource，范围不包括时长、tag等.  
如果不使用条件排序则使用`推荐系统排序`，指定使用私有推荐系数.
### 过滤
返回含有`过滤`的所有元素的resource，范围包括Tag、发布时期、地区、resource类型.
### 条件排序
根据resource的单个字段进行排序，可选的字段和resource类型有关.  
与推荐系统排序互斥，两者仅可选其一.

## 拓展功能
### 搜索范围限定
拓展自：[关键词搜索](#关键词搜索)  
限定关键词搜索的resource的键.
### 单类resource
获取同一类别的resource.
### 推荐系统排序
配合[推荐系统](../../recommendation/design.md)，通过用户的推荐数据对过滤后的结果进行排序.  
与条件排序互斥，两者仅可选其一.  
必须指定使用公有推荐系数或私有推荐系数.

## 边沿功能
### 模糊搜索
拓展自：[关键词搜索](#关键词搜索)  
通过人工制作的近似字典和简单错拼算法进行搜索，前者解决语义相似，后者解决错误拼写.